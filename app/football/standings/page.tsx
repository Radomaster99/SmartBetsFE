...
function buildFlagMap(countries: CountryDto[] | undefined): Map<string, string> {
  ...
  countries?.forEach((c) => { if (c.flagUrl) map.set(normalizeCountryName(c.name), c.flagUrl); });
  return map;
}
...
function LeaguePicker({
  ...
  const [adminPresets, setAdminPresets] = useState<PopularLeaguePreset[]>([]);
  const [userPresets, setUserPresets] = useState<PopularLeaguePreset[]>([]);
  const [hiddenKeys, setHiddenKeys] = useState<string[]>([]);
  const [hydrated, setHydrated] = useState(false);

  useEffect(() => {
    setAdminPresets(readPopularLeaguePresets(ADMIN_POPULAR_LEAGUES_STORAGE_KEY, DEFAULT_POPULAR_LEAGUES_PRESET));
    ...
    <div style={{ maxWidth: 860, margin: '0 auto' }}>
      ...
      {normalizedSearch ? (
        ...
        <>
          ...
          {/* Popular (admin defaults, minus user pinned) */}
      ...
      >
        <div>
          ...
          <p className="text-[12px]" style={{ color: 'var(--t-text-5)' }}>
            {leagueId ? 'Click any team to open its detail page.' : 'Select a league to view its table.'}
          </p>
        </div>
        ...
        {!leagueId ? (
          ...
          <StandingsTable
            ...
            resolveTeamHref={(standing) =>
              leagueId && standing.apiTeamId ? buildTeamHref(standing.apiTeamId, leagueId, season) : null
            }
          />
        )}
      </div>
